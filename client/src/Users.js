...
function App() {
  ...
  const [newMember, setNewMember] = useState({ member_name: "", email: "" }); // Member
  // const [newProduct, setNewProduct] = useEffect({item_name: "",price: "",});
  ...
  setNewMember((prevMember) => ({...prevMember, [name]: value})); 여기서 작성한 name과 value는 input 안에 있는 name = 과 value = 를 의미 !
  name에서 member_name은 http://localhost:8080/api/member에 json  참조하여 작성한 member_name으로 값을 변경하면 안됨.
  */
